...
void UI_update_rssi(const int rssi, const unsigned int glitch, const unsigned int noise, const unsigned int vfo)
{
	...
	{	// original little RSSI bars
		...
		/* Make the following control flow explicit to avoid
		 -Wmisleading-indentation when preprocessor blocks surround conditionals **/
		if (g_current_function == FUNCTION_TRANSMIT || g_current_display_screen != DISPLAY_MAIN)
		{
			return;    // display is in use
		}

		/* Now it's unambiguous that memset is not part of the previous if */
		memset(pline, 0, 23);
	...
	{	// top line
			...
			#endif
		{
			...
			{	// frequency band/channel number
				...
				else if (IS_FREQ_CHANNEL(scrn_chan))
				{	// frequency mode
...

	{
		...
		else
		{
			...
			{	// a user channel
				...
				// frequency: suppressed intentionally for channel mode
				#if 0
...
void UI_DisplayMain(void)
{
	...
	for (vfo_num = 0; vfo_num < 2; vfo_num++)
	{
		...
		/* make this explicit so the following if/ifdef blocks don't make
		 the* indentation misleading to the compiler */
		...
		if (current_vfo_num != vfo_num)
		{
		 ...
		 if (single_vfo < 0)
		 {
			 ...
			 else if (g_eeprom.config.setting.cross_vfo != CROSS_BAND_OFF || vfo_num == current_vfo_num)
			 {
...

		 {
			 ...
			 else if (g_input_box_index > 0 && IS_FREQ_CHANNEL(scrn_chan) && g_eeprom.config.setting.tx_vfo_num == vfo_num)
			 {	// user is entering a frequency
			 ...
			 else
			 {
				 ...
				 {	// a user channel
					 ...
					 {
						 case MDF_FREQUENCY:	// originally: just channel frequency
							 // Frequency suppressed for channel mode per request: show channel number instead
						 ...
						 case MDF_NAME_FREQ:	// channel name and frequency
							 // Show name only for channel mode (do not print numeric frequency)
							 SETTINGS_fetch_channel_name(str, scrn_chan);
							 if (str[0] == 0)
							 ...
							 // show name (omit frequency even for MDF_NAME_FREQ)
							 #ifdef ENABLE_SMALL_BOLD
		 ...
		 {	// show the TX/RX level
			 ...
			 /* make the call a simple statement (no dangling else) */
			 {
